...
dummies=pd.get_dummies(holdout["Embarked"],prefix="Embarked")
...
extracted_titles=train["Name"].str.extract(' ([A-Z][a-z]+)\.',expand=False)
train["Title"]=extracted_titles.map(titles)
extracted_titles=holdout["Name"].str.extract(' ([A-Z][a-z]+)\.',expand=False)
...
train=pd.concat([train,dummies],axis=1)
dummies=pd.get_dummies(train["Cabin_type"], prefix="Cabin_type")
train=pd.concat([train,dummies],axis=1)

dummies=pd.get_dummies(holdout["Title"], prefix="Title")
holdout=pd.concat([holdout,dummies],axis=1)
dummies=pd.get_dummies(holdout["Cabin_type"], prefix="Cabin_type")
holdout=pd.concat([holdout,dummies],axis=1)
